...
function createCard(book, num) {
	const { title, sub, text } = book;
	const div = create('div');
	const subEl = create('p', { innerText: sub });
	const titleEl = create('h1', { innerText: title });
	const textEl = create('div', { innerText: text });
	const counterEl = create('div', { innerText: num, className: 'counter' });
	const btn = create('button', { className: 'btn' });
	const textSpan = create('span', { innerText: 'read more' });
	const iconSpan = create('span', { className: 'material-icons', innerText:  'arrow_forward' });
 
	btn.append(textSpan, iconSpan);
	div.append(subEl, titleEl, textEl, counterEl, btn);
	return div;
 }
 ...
 function main() {
   ...
   for (let i = 1; i < books.cards.length; i++) {
     const card = createCard(books.cards[i], i);
     cnt.append(card);
   }
 }
